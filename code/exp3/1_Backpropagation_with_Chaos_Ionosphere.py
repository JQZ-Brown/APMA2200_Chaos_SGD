...
Description:
    ...
    Implement classification tasks on the Ionosphere dataset.
...
Reference:
    ...
    2. Ionosphere dataset: https://archive.ics.uci.edu/dataset/52/ionosphere
'''
import copy
...
# Load data
data = pd.read_csv("../../data/ionosphere/ionosphere.data", sep=",", header=None, index_col=None).values
X = np.asarray(data[:, :-1], dtype=float)
X = np.delete(X, 1, 1) # remove the first attribute since its values are all zero
Y = data[:, -1]
...
Y_onehot[np.where(Y == "b")[0], 0] = 1.0
Y_onehot[np.where(Y == "g")[0], 1] = 1.0
Y = Y_onehot
...
np.save("./res/BP_Chaos/Ionosphere-acc.npy", {
